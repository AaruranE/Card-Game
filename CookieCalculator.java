...
// Cookie Calculator by Aaruran Elamurugaiyan and Neeilan Selvalingam
...
public class CookieCalculator extends Applet implements MouseListener, MouseMotionListener //CookieCalculator
{
    ...
    {
        for (int i = 0 ; i < 9 ; i++)
            numbers [i] = (int) (Math.random () * 899) + 100;
    ...
    { // called after a button is pressed down
        ...
        e.consume (); //ensures the other mouse methods don't call on this event
    }
    ...
    public boolean action (Event e, Object o)
    {
        if (e.target == Submit)
        {
            ...
            for (int i = 0 ; i < 9 ; i++)
            {
                if (HasItBeenClicked [i] != (numbers [i] % typeOfNum == 0))
                {
                    isItRight = false;
                    break;
                }
    ...
    { //called when the user clicks the mouse withint the applet's rectangular area
    }


    public void mouseDragged (MouseEvent e)
    { // called during motion while buttons are clicked
    }
